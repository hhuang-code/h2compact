...
def run(cfg,
    ...
    with torch.no_grad():
        if not (osp.exists(osp.join(output_pth, 'tracking_results.pth')) and
            ...
            tracking_results = detector.process(fps)    # dict_keys(['frame_id', 'bbox', 'keypoints'])
            ...
            # ['frame_id', 'bbox', 'keypoints', 'features', 'init_global_orient', 'init_body_pose', 'init_betas',
            # 'flipped_bbox', 'flipped_keypoints', 'flipped_features', 'flipped_init_global_orient',
            # 'flipped_init_body_pose', 'flipped_init_betas']
    ...
    for subj in range(n_subjs):
        ...
        pred_root_world = matrix_to_axis_angle(pred['poses_root_world']).cpu().numpy().reshape(-1, 3)   # orientation in global frame
        pred_pose = np.concatenate((pred_root, pred_body_pose), axis=-1)
        pred_pose_world = np.concatenate((pred_root_world, pred_body_pose), axis=-1)
        pred_trans = (pred['trans_cam'] - network.output.offset).cpu().numpy()

        results[_id]['pose'] = pred_pose    # (n_frames, 72)
        results[_id]['trans'] = pred_trans  # (n_frames, 3)
        results[_id]['pose_world'] = pred_pose_world    # (n_frames, 72)
        results[_id]['trans_world'] = pred['trans_world'].cpu().squeeze(0).numpy()  # (n_frames, 3), translation in global frame
        results[_id]['betas'] = pred['betas'].cpu().squeeze(0).numpy()  # (n_frames, 10)
        results[_id]['verts'] = (pred['verts_cam'] + pred['trans_cam'].unsqueeze(1)).cpu().numpy()  # (n_frames, n_vertices, 3)
        results[_id]['frame_ids'] = frame_id    # (n_frames)
